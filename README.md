...
5. To allow ChugBot to archive the current summer's data, use your ISP's database admin tool (or the mysql command-line tool) to create a database called DB2016, where DB is the name of your main database from step (1).  For example, if your database is called camprama_chugbot_db, and the current summer year is 2016, you would create camprama_chugbot_db2016.  It's a good idea to create a few of these for the coming years (camprama_chugbot_db2017, camprama_chugbot_db2018, etc).  These are used by ChugBot when you archive your current data, which is done from the archive.php page.  The archive databases have to be created this way because historically, the user ChugBot runs as was not allowed to create databases (if your site allows ChugBot's user to create databases, it would be easy to modify archive.php to create the archive database on the fly).
